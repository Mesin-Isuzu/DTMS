...
class App {
    ...
    _supplierIdByName(name) {
        ...
        if (!key) return null;
        const sp = (this.data.suppliers || []).find(x => (x.name || '').trim().toLowerCase() === key);
        if (sp && sp.supplierId) return sp.supplierId;
        const map = {
    ...
    _syncSupplierIdField(prefix) {
        const supplierEl=document.getElementById(prefix+'-supplier');
        const sidEl=document.getElementById(prefix+'-supplierid');
        if(!sidEl) return;
        const name=(supplierEl?.value||'').trim();
        if(!name) return;
    ...
    _onToolingSupplierChange(prefix) {
        const supplierEl=document.getElementById(prefix+'-supplier');
        if(!supplierEl) return;
        const name=(supplierEl.value||'').trim();
        const sp=(this.data.suppliers||[]).find(x=>x.name===name);
        const set=(id,val)=>{const el=document.getElementById(prefix+'-'+id);if(el)el.value=val||'';};
        set('supplierid', sp?sp.supplierId:(this._supplierIdByName(name)||''));
        set('supplierAddress', sp?sp.address:'');
        set('mapUrl', sp?sp.mapUrl:'');
        set('pic', sp?sp.pic:'');
        set('picEmail', sp?sp.picEmail:'');
        set('picPhone', sp?sp.picPhone:'');
    }

    _supplierSelectOptions(prefix, currentName) {
        const list=this.data.suppliers||[];
        const names=list.map(x=>x.name);
        if(currentName && !names.includes(currentName)) names.push(currentName);
        const opts=names.map(n=>{const sp=list.find(x=>x.name===n);const label=sp?`${n} (${sp.supplierId})`:n;return `<option value="${n}"${n===currentName?' selected':''}>${label}</option>`;}).join('');
        return `<option value=""${currentName?'':' selected'}>— Pilih Supplier —</option>${opts}`;
    }
...
getToolingListView() {
    ...
    getReportsView() {
        ...
        t.forEach(tool=>{
            ...
            const qtyPer=this.parseFormattedNumber(tool.qtyPerTooling)||1;
            sumReject+=cumShoot>0?((1-matchedQtyOk/(cumShoot*qtyPer))*100):0;
            const qtyDep=this.parseFormattedNumber(tool.qtyDepreciation)||0;
    ...
    exportToolingExcel() {
        ...
        const rows=toolings.map((t,i)=>{
            ...
            const qtyPerTooling=this.parseFormattedNumber(t.qtyPerTooling)||1;
            ...
            const qtyDep=this.parseFormattedNumber(t.qtyDepreciation)||0;
            const sisa=qtyDep-totalQtyOk;
            const lifeRatio=(cumShoot/(this.parseFormattedNumber(t.maxShoot)||1)*100).toFixed(1);
            return [
    ...
    async submitAddTooling() {
        ...
        const supplierId = v('supplierid') || this._supplierIdByName(v('supplier'));
    ...
    openEditToolingModal(toolId) {
        ...
        const ownerVals=['Milik MII','Milik Supplier','Milik Pelanggan'];
        if(t.owner && !ownerVals.includes(t.owner)) ownerVals.push(t.owner);
        const ownerOpts=ownerVals.map(o=>`<option ${o===t.owner?'selected':''}>${o}</option>`).join('');
        const typeMaster=(this.data.dieTypes||[]).map(x=>x.name);
        const modelMaster=(this.data.productModels||[]).map(x=>x.name);
        ...
        modal.innerHTML=`<div class="modal-content" style="max-width:640px;max-height:90vh"><div class="modal-header"><h3 class="modal-title"><i class="fas fa-edit" style="color:var(--accent-color);margin-right:0.5rem"></i>Ubah Tooling: ${toolId}</h3><button class="modal-close" onclick="app.closeModal('edit-tooling-modal')">&times;</button></div><div class="modal-body" style="overflow-y:auto"><div style="display:grid;grid-template-columns:1fr 1fr;gap:0 1rem">${fg('name','Nama',t.name)}<div class="form-group"><label class="form-label">Tipe</label><select id="et-type" class="form-control">${typeOpts}</select></div>${fg('pn','Part Number',t.partNumber)}${fg('partName','Nama Part',t.partName)}<div class="form-group"><label class="form-label">Status</label><select id="et-status" class="form-control">${statOpts}</select></div><div class="form-group"><label class="form-label">Kondisi</label><select id="et-cond" class="form-control">${condOpts}</select></div><div class="form-group"><label class="form-label">Model</label><select id="et-model" class="form-control">${modelOpts}</select></div><div class="form-group"><label class="form-label">Supplier</label><select id="et-supplier" class="form-control" onchange="app._onToolingSupplierChange('et')">${this._supplierSelectOptions('et', t.supplier)}</select></div><div class="form-group"><label class="form-label">Supplier ID</label><input type="text" id="et-supplierid" class="form-control" value="${t.supplierId||this._supplierIdByName(t.supplier)||''}" placeholder="Terisi otomatis dari Supplier"></div>${fg('supplierAddress','Alamat Supplier',t.supplierAddress)}${fg('mapUrl','Link Google Maps',t.mapUrl)}<div class="form-group"><label class="form-label">Kepemilikan</label><select id="et-owner" class="form-control">${ownerOpts}</select></div>${fg('maker','Maker',t.maker)}${fg('weight','Berat',t.weight)}${fg('tonnage','Tonase',t.tonnage)}${fg('dimensions','Dimensi (PxLxT)',t.dimensions)}${fg('material','Material',t.material)}${fgCur('price','Harga',t.price)}${fg('pic','PIC',t.pic)}${fg('picEmail','Email PIC',t.picEmail)}${fg('picPhone','Telepon PIC',t.picPhone)}${fgNum('maxShoot','Maximum Shoot',t.maxShoot)}${fgNum('qtyPerTooling','Qty Part per Tooling',this.parseFormattedNumber(t.qtyPerTooling))}${fg('paNumber','No. PO/Tooling PA',t.paNumber)}${!this.currentUser.role.includes('Supplier') ? fgNum('qtyDepreciation','QTY Depresiasi (pcs)',this.parseFormattedNumber(t.qtyDepreciation))+fg('depreciationValue','Periode Depresiasi (tahun)',t.depreciationValue||'') : ''}</div></div><div class="modal-footer">${this.currentUser.role.includes('Admin') ? `<button type="button" class="btn btn-secondary" onclick="app.closeModal('edit-tooling-modal');app.openDrawingDiesModal('${toolId}')"><i class="fas fa-upload"></i> Upload Drawing</button>` : ''}<button class="btn btn-secondary" onclick="app.closeModal('edit-tooling-modal')">Batal</button><button class="btn btn-primary" onclick="app.submitEditTooling('${toolId}')"><i class="fas fa-save"></i> Simpan</button></div></div>`;
        document.body.appendChild(modal); document.body.style.overflow='hidden';
    ...
    async submitEditTooling(toolId) {
        ...
        const newSupplierId=v('supplierid')||this._supplierIdByName(newSupplier);
        ...
        t.owner=document.getElementById('et-owner')?.value||t.owner;
        t.name=v('name')||t.name; t.type=v('type')||t.type; t.partNumber=v('pn')||t.partNumber; t.partName=v('partName')||t.partName; t.status=v('status')||t.status; t.condition=v('cond')||t.condition; t.model=v('model')||t.model; t.supplier=newSupplier; t.supplierId=newSupplierId; t.supplierAddress=v('supplierAddress')||t.supplierAddress; t.mapUrl=v('mapUrl')||t.mapUrl; t.maker=v('maker')||t.maker; t.weight=v('weight')||t.weight; t.tonnage=v('tonnage')||t.tonnage; t.dimensions=v('dimensions')||t.dimensions; t.material=v('material')||t.material; {const pv=v('price'); t.price=pv?this.formatCurrency(this.parseCurrency(pv)):t.price;} t.pic=v('pic')||t.pic; t.picEmail=v('picEmail')||t.picEmail; t.picPhone=v('picPhone')||t.picPhone;         t.maxShoot=this.parseFormattedNumber(v('maxShoot'))||t.maxShoot; t.qtyPerTooling=this.parseFormattedNumber(v('qtyPerTooling'))?this.formatNumber(this.parseFormattedNumber(v('qtyPerTooling'))):t.qtyPerTooling; t.paNumber=v('paNumber')||t.paNumber; if(!this.currentUser.role.includes('Supplier')){t.qtyDepreciation=this.parseFormattedNumber(v('qtyDepreciation'))?this.formatNumber(this.parseFormattedNumber(v('qtyDepreciation'))):t.qtyDepreciation; t.depreciationValue=v('depreciationValue')||'';}
        if(window.DTMS && window.DTMS.enabled()){
    ...
    openAddSupplierModal() {
        if (!this.currentUser.role.includes('Admin')) return;
        const list=this.data.suppliers||[];
        const maxSup=list.map(x=>parseInt(String(x.supplierId||'').replace(/[^0-9]/g,''))||0).reduce((m,n)=>Math.max(m,n),0);
        const nextId=`SUP${String(maxSup+1).padStart(3,'0')}`;
        const modal=document.createElement('div');
        modal.id='add-supplier-modal'; modal.className='modal-overlay'; modal.style.cssText='display:flex;opacity:1;visibility:visible;';
        modal.innerHTML=`<div class="modal-content" style="max-width:520px"><div class="modal-header"><h3 class="modal-title"><i class="fas fa-plus-circle" style="color:var(--accent-color);margin-right:0.5rem"></i>Tambah Supplier</h3><button class="modal-close" onclick="app.closeModal('add-supplier-modal')">&times;</button></div><div class="modal-body"><div style="display:grid;grid-template-columns:1fr 1fr;gap:0 1rem"><div class="form-group"><label class="form-label">Supplier ID <span style="color:var(--danger-color)">*</span></label><input type="text" id="sp-supplierid" class="form-control" value="${nextId}" placeholder="Contoh: SUP004"></div><div class="form-group"><label class="form-label">Nama Supplier <span style="color:var(--danger-color)">*</span></label><input type="text" id="sp-name" class="form-control" placeholder="Contoh: PT Chemco Indonesia"></div><div class="form-group" style="grid-column:1/-1"><label class="form-label">Alamat Supplier</label><input type="text" id="sp-address" class="form-control" placeholder="Contoh: Jl. Industri No. 10"></div><div class="form-group" style="grid-column:1/-1"><label class="form-label">Link Google Maps</label><input type="text" id="sp-map" class="form-control" placeholder="https://www.google.com/maps/embed?pb=..."></div><div class="form-group"><label class="form-label">Nama PIC</label><input type="text" id="sp-pic" class="form-control" placeholder="Contoh: Budi"></div><div class="form-group"><label class="form-label">Email PIC</label><input type="text" id="sp-picemail" class="form-control" placeholder="Contoh: budi@supplier.com"></div><div class="form-group" style="grid-column:1/-1"><label class="form-label">Telepon PIC</label><input type="text" id="sp-picphone" class="form-control" placeholder="Contoh: +62 812-3456-7890"></div></div></div><div class="modal-footer"><button class="btn btn-secondary" onclick="app.closeModal('add-supplier-modal')">Batal</button><button class="btn btn-primary" onclick="app.submitAddSupplier()"><i class="fas fa-save"></i> Simpan</button></div></div>`;
        document.body.appendChild(modal); document.body.style.overflow='hidden';
    }

    async submitAddSupplier() {
        if (!this.currentUser.role.includes('Admin')) return;
        const v=id=>document.getElementById('sp-'+id)?.value?.trim()||'';
        const list=this.data.suppliers||(this.data.suppliers=[]);
        if(!v('supplierid')||!v('name')){alert('Supplier ID dan Nama Supplier wajib diisi.');return;}
        if(list.some(x=>String(x.supplierId).toLowerCase()===v('supplierid').toLowerCase())){alert(`Supplier ID "${v('supplierid')}" sudah ada.`);return;}
        if(list.some(x=>(x.name||'').toLowerCase()===v('name').toLowerCase())){alert(`Nama supplier "${v('name')}" sudah ada.`);return;}
        const newSupplier={id:list.reduce((m,x)=>Math.max(m,x.id||0),0)+1,supplierId:v('supplierid'),name:v('name'),address:v('address'),mapUrl:v('map'),pic:v('pic'),picEmail:v('picemail'),picPhone:v('picphone')};
        if(window.DTMS && window.DTMS.enabled()){
            try{const created=await window.DTMS.insertSupplier(newSupplier);if(created&&created.id)newSupplier.id=created.id;}catch(e){console.error(e);alert('Gagal menyimpan supplier ke database: '+(e?.message||e));return;}
        }
    ...
    openEditSupplierModal(id) {
        if (!this.currentUser.role.includes('Admin')) return;
        const s=(this.data.suppliers||[]).find(x=>x.id===id); if(!s) return;
        const modal=document.createElement('div');
        modal.id='edit-supplier-modal'; modal.className='modal-overlay'; modal.style.cssText='display:flex;opacity:1;visibility:visible;';
        modal.innerHTML=`<div class="modal-content" style="max-width:520px"><div class="modal-header"><h3 class="modal-title"><i class="fas fa-edit" style="color:var(--accent-color);margin-right:0.5rem"></i>Ubah Supplier: ${s.name}</h3><button class="modal-close" onclick="app.closeModal('edit-supplier-modal')">&times;</button></div><div class="modal-body"><div style="display:grid;grid-template-columns:1fr 1fr;gap:0 1rem"><div class="form-group"><label class="form-label">Supplier ID <span style="color:var(--danger-color)">*</span></label><input type="text" id="esp-supplierid" class="form-control" value="${s.supplierId||''}"></div><div class="form-group"><label class="form-label">Nama Supplier <span style="color:var(--danger-color)">*</span></label><input type="text" id="esp-name" class="form-control" value="${s.name||''}"></div><div class="form-group" style="grid-column:1/-1"><label class="form-label">Alamat Supplier</label><input type="text" id="esp-address" class="form-control" value="${s.address||''}"></div><div class="form-group" style="grid-column:1/-1"><label class="form-label">Link Google Maps</label><input type="text" id="esp-map" class="form-control" value="${s.mapUrl||''}"></div><div class="form-group"><label class="form-label">Nama PIC</label><input type="text" id="esp-pic" class="form-control" value="${s.pic||''}"></div><div class="form-group"><label class="form-label">Email PIC</label><input type="text" id="esp-picemail" class="form-control" value="${s.picEmail||''}"></div><div class="form-group" style="grid-column:1/-1"><label class="form-label">Telepon PIC</label><input type="text" id="esp-picphone" class="form-control" value="${s.picPhone||''}"></div></div></div><div class="modal-footer"><button class="btn btn-secondary" onclick="app.closeModal('edit-supplier-modal')">Batal</button><button class="btn btn-primary" onclick="app.submitEditSupplier(${id})"><i class="fas fa-save"></i> Simpan</button></div></div>`;
        document.body.appendChild(modal); document.body.style.overflow='hidden';
    }

    async submitEditSupplier(id) {
        if (!this.currentUser.role.includes('Admin')) return;
        const s=(this.data.suppliers||[]).find(x=>x.id===id); if(!s) return;
        const v=id=>document.getElementById('esp-'+id)?.value?.trim()||'';
        if(!v('supplierid')||!v('name')){alert('Supplier ID dan Nama Supplier wajib diisi.');return;}
        const dupSid=(this.data.suppliers||[]).some(x=>x.id!==id&&String(x.supplierId).toLowerCase()===v('supplierid').toLowerCase());
        if(dupSid){alert(`Supplier ID "${v('supplierid')}" sudah digunakan supplier lain.`);return;}
        const prev={supplierId:s.supplierId,name:s.name,address:s.address,mapUrl:s.mapUrl,pic:s.pic,picEmail:s.picEmail,picPhone:s.picPhone};
        s.supplierId=v('supplierid'); s.name=v('name'); s.address=v('address'); s.mapUrl=v('map'); s.pic=v('pic'); s.picEmail=v('picemail'); s.picPhone=v('picphone');
        if(window.DTMS && window.DTMS.enabled()){
            try{await window.DTMS.updateSupplier(id, s);}catch(e){console.error(e);Object.assign(s,prev);alert('Gagal memperbarui supplier di database: '+(e?.message||e));return;}
        }
    ...
    async submitDeleteSupplier(id) {
        ...
        if(idx===-1)return;
        const item=this.data.suppliers[idx];
        const usage=this.data.toolings.filter(t=>t.supplierId===item.supplierId).length;
        if(usage>0){alert(`Supplier "${item.name}" tidak dapat dihapus karena masih digunakan oleh ${usage} tooling.`);return;}
        if(!confirm(`Yakin ingin menghapus supplier "${item.name}" (${item.supplierId})?`))return;
        if(window.DTMS && window.DTMS.enabled()){
            try{await window.DTMS.deleteSupplier(id);this.data.suppliers.splice(idx,1);}catch(e){console.error(e);alert('Gagal menghapus supplier dari database: '+(e?.message||e));return;}
        } else {
    ...
    openShootLogModal(toolId, period = 'monthly') {
        ...
        const maxLife = t.maxShoot || 1000000;
        const isSupplier = this.currentUser.role === 'Pengguna Supplier';
        const canManageShoot = isSupplier || this.currentUser.role.includes('Admin');
        ...
        const rows = logs.map((l, idx) => {
            ...
            return `<tr>
                ...
                ${canManageShoot && !isAggregated ? (l.id
        ...
        modal.innerHTML = `<div class="modal-content" style="max-width:${modalW}px"><div class="modal-header"><h3 class="modal-title"><i class="fas fa-chart-line" style="color:var(--accent-color);margin-right:0.5rem"></i>Riwayat Shoot � ${t.id} (${t.name})</h3><button class="modal-close" onclick="app.closeModal('shoot-log-modal')">&times;</button></div><div class="modal-body" style="max-height:80vh;overflow-y:auto">
            <div style="margin-bottom:0.75rem;display:flex;justify-content:space-between;align-items:center">
                ...
                ${canManageShoot && !isAggregated ? `<button class="btn btn-primary btn-sm" onclick="app.openAddShootModal('${toolId}')"><i class="fas fa-plus"></i> Tambah Shoot</button>` : ''}
            </div>
            ...
            <div id="shoot-table-wrap" class="table-responsive"><table class="table" style="margin-bottom:0"><thead><tr><th>No.</th><th>${isAggregated ? 'Periode' : 'Tanggal Pengisian'}</th><th>Jumlah Shoot</th><th>Kumulatif Shoot</th><th>Life Tool Ratio</th>${canManageShoot && !isAggregated ? '<th>Aksi</th>' : ''}</tr></thead><tbody>${rows || `<tr><td colspan="${canManageShoot && !isAggregated ? 6 : 5}" style="text-align:center;padding:2rem;color:var(--text-secondary)">Belum ada data riwayat shoot.</td></tr>`}</tbody></table></div>
